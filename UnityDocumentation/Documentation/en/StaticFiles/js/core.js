...
/*    // Change prefered scripting lanuage
    $('.script-lang li').click(function() {
        var lang = $(this).attr('data-lang');
        storeValue('scriptlangdialog', 'yes');
        checkScriptLanguage(lang);
        $('#script-lang-dialog').hide();
    });

    // Close prefered scripting lanuage dialog
    $('#script-lang-dialog .close').click(function() {
        storeValue('scriptlangdialog', 'yes');
        $('#script-lang-dialog').hide();
    });
...
function getCurrentVersion() {
    try {
        ...
        var openPage = list[list.length - 2] + "/" + list[list.length - 1];
